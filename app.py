...
section.main, .stMain, .main {
    background: radial-gradient(ellipse at 50% 0%,
        #0d1b4b 0%, #05091a 55%, #000000 100%) !important;
...
.nasa-header-left   { display: flex; align-items: center; gap: 0.85rem; }
...
.nasa-header-subtitle {
    font-size: 0.73rem; color: #6090c0; margin: 0.1rem 0 0 0;
    letter-spacing: 0.06em; font-family: 'Share Tech Mono', monospace !important;
}
.nasa-header-badge {
    background: rgba(40,100,255,0.12);
    border: 1px solid rgba(80,160,255,0.35);
    border-radius: 4px; padding: 0.3rem 0.85rem;
    font-size: 0.68rem; color: #80c0ff; letter-spacing: 0.08em;
...
.data-panel {
    ...
    border-left: 1px solid rgba(40,100,255,0.20);
...
.panel-empty-wrap {
    ...
    min-height: 460px; padding: 2rem; text-align: center;
}
.panel-empty-icon  { font-size: 2.8rem; margin-bottom: 1rem; }
.panel-empty-title { font-size: 0.9rem; color: #4a7aaa; line-height: 1.6; font-weight: 500; }
.panel-empty-sub   {
    font-size: 0.72rem; color: #2a4a6a; margin-top: 0.6rem;
    letter-spacing: 0.04em; font-family: 'Share Tech Mono', monospace !important;
}

/* ── City header ── */
.city-block {
    padding: 1.1rem 1.3rem 0.9rem 0;
    border-bottom: 1px solid rgba(40,100,255,0.15);
}
.city-name {
    font-size: 1.3rem; font-weight: 700; color: #e8f4ff;
    margin: 0 0 0.15rem 0; letter-spacing: 0.02em;
    text-shadow: 0 0 12px rgba(80,160,255,0.50);
}
.city-coords {
    font-size: 0.70rem; color: #3a6a9a;
    font-family: 'Share Tech Mono', monospace !important;
    letter-spacing: 0.06em;
}

/* ── AQI orb ── */
.aqi-block { padding: 1rem 0 1rem 0; border-bottom: 1px solid rgba(40,100,255,0.12); }
.aqi-row   { display: flex; align-items: center; gap: 1rem; }
.aqi-orb {
...
.aqi-sublabel {
    font-size: 0.70rem; color: #4a7aaa; margin-top: 0.1rem;
    letter-spacing: 0.05em; font-family: 'Share Tech Mono', monospace !important;
}
...
.metric-card {
    ...
    box-shadow: 0 0 12px rgba(40,100,255,0.08),
                inset 0 0 8px rgba(40,100,255,0.04);
}
.mc-label {
    font-size: 0.60rem; color: #2a6a9a; font-weight: 600;
    text-transform: uppercase; letter-spacing: 0.10em; margin-bottom: 0.2rem;
    font-family: 'Share Tech Mono', monospace !important;
}
.mc-value {
    font-size: 1.1rem; font-weight: 700; color: #a0d0ff;
    font-family: 'Share Tech Mono', monospace !important; line-height: 1.1;
}
.mc-unit { font-size: 0.58rem; color: #2a5a7a; font-family:'Share Tech Mono',monospace !important; }

/* ── WHO panel ── */
.who-panel { padding: 0.85rem 0; border-bottom: 1px solid rgba(40,100,255,0.12); }
.panel-title {
    font-size: 0.65rem; font-weight: 700; text-transform: uppercase;
    letter-spacing: 0.12em; color: #2a6a9a; margin-bottom: 0.6rem;
    font-family: 'Share Tech Mono', monospace !important;
}
.who-row {
    display: flex; align-items: center;
    justify-content: space-between; margin-bottom: 0.5rem; gap: 0.4rem;
}
...
.who-note  { font-size: 0.60rem; color: #2a4a6a; margin-top: -0.3rem; margin-bottom: 0.3rem; font-style: italic; }

/* ── Health panel ── */
.health-panel { padding: 0.85rem 0; border-bottom: 1px solid rgba(40,100,255,0.12); }
.health-box {
    border-radius: 5px; padding: 0.7rem 0.9rem;
    font-size: 0.76rem; line-height: 1.6; color: #d0e8ff;
    border-left: 3px solid;
}

/* ── Chart section ── */
.chart-section  { padding: 0.7rem 0 0.4rem 0; }
.chart-label {
    font-size: 0.63rem; font-weight: 700; text-transform: uppercase;
    letter-spacing: 0.12em; color: #2a6a9a; margin-bottom: 0.3rem;
    font-family: 'Share Tech Mono', monospace !important;
}

/* ── AQI scale strip ── */
.scale-strip { display: flex; flex-wrap: wrap; gap: 0.25rem; padding: 0.2rem 0 0.8rem 0; }
.scale-label-head {
    font-size: 0.58rem; color: #2a5a7a; width: 100%;
    font-weight: 700; text-transform: uppercase; letter-spacing: 0.09em;
    margin-bottom: 0.05rem; font-family: 'Share Tech Mono', monospace !important;
}
.scale-chip {
...
.preview-panel {
    background: rgba(3,8,25,0.90);
    border: 1px solid rgba(40,120,255,0.35);
    border-radius: 8px;
    overflow: hidden;
    box-shadow: 0 0 24px rgba(40,100,255,0.18), 0 0 48px rgba(20,60,200,0.10);
    margin-top: 1.5rem;
}
.preview-titlebar {
    background: linear-gradient(90deg, #050d2a 0%, #0a1838 100%);
    border-bottom: 1px solid rgba(40,120,255,0.28);
...
.preview-title {
    font-size: 0.72rem; font-weight: 600; color: #80c0ff;
    font-family: 'Share Tech Mono', monospace !important;
    letter-spacing: 0.06em;
}
...
.preview-footer {
    ...
    border-top: 1px solid rgba(40,120,255,0.20);
    padding: 0.55rem 0.9rem;
    text-align: right;
}
.preview-footer a {
    font-size: 0.72rem; color: #60a8ff; text-decoration: none;
    font-family: 'Share Tech Mono', monospace !important;
    letter-spacing: 0.04em; border-bottom: 1px solid rgba(96,168,255,0.40);
    padding-bottom: 1px;
}
.preview-footer a:hover { color: #90c8ff; border-bottom-color: rgba(144,200,255,0.60); }

/* ── Map note ── */
.map-note {
    font-size: 0.70rem; color: #2a5a8a; text-align: center;
...
.disclaimer {
    ...
    font-size: 0.72rem; color: #5a7a9a; line-height: 1.65;
...
.space-footer {
    ...
    text-align: center; padding: 1.1rem 1.5rem 1.2rem 1.5rem;
    position: relative; z-index: 2;
}
.footer-brand {
...
.footer-tagline { font-size: 0.73rem; color: #5a8aaa; margin-top: 0.2rem; letter-spacing: 0.03em; }
.footer-link    { font-size: 0.72rem; margin-top: 0.35rem; }
.footer-link a  {
    color: #60a8ff; text-decoration: underline;
    text-underline-offset: 2px; letter-spacing: 0.03em;
}
...
WHO_GUIDELINES = {"PM2.5": 15.0, "PM10": 45.0, "NO₂": 25.0}
...
def get_health_advice(v):
    try:
        v = float(v)
        if np.isnan(v):
            return "#334455", "rgba(30,50,90,0.12)", "No AQI data available."
    except (TypeError, ValueError):
        return "#334455", "rgba(30,50,90,0.12)", "No AQI data available."
...
# ─────────────────────────────────────────────────────────────────────────────
#  FOLIUM MAP  —  FIX 1: bold white labels, white-outline markers
# ─────────────────────────────────────────────────────────────────────────────
def build_map(selected_city=None, aqi_cache=None):
    ...
    # AQI legend (top-right)
    ranges_str = ["0–20", "21–40", "41–60", "61–80", "81–100", "100+"]
    legend_html = """
    <div style="position:absolute;top:12px;right:12px;z-index:9999;
        background:rgba(2,5,20,0.93);border:1px solid rgba(40,120,255,0.30);
        border-radius:6px;padding:10px 14px;
        box-shadow:0 0 20px rgba(20,80,200,0.22);">
      <div style="font-size:10px;font-weight:800;text-transform:uppercase;
                  letter-spacing:0.12em;color:#4090cc;margin-bottom:8px;
    ...
    for i, (thr, col, lbl) in enumerate(AQI_SCALE):
        legend_html += (
            ...
            f'box-shadow:0 0 6px {col};'
            f'border:1.5px solid rgba(255,255,255,0.40);"></span>'
            f'<span style="font-size:10.5px;color:#c0dff8;font-family:\'Share Tech Mono\',monospace;">'
    ...
    for city, info in CITIES.items():
        ...
        # FIX 1: tooltip with dark background + bold white text
        tooltip_html = (
            f"<div style='"
            f"background:rgba(26,26,46,0.92);"
            f"border:1px solid rgba(80,160,255,0.40);"
            f"border-radius:6px;"
            f"padding:6px 10px;"
            f"font-family:\"Exo 2\",sans-serif;'>"
            f"<span style='font-size:13px;font-weight:700;color:#ffffff;"
            f"text-shadow:0 0 8px rgba(80,160,255,0.80);display:block;'>{city}</span>"
            f"<span style='font-size:10px;color:#80b8e8;"
            f"font-family:\"Share Tech Mono\",monospace;display:block;margin-top:2px;'>"
            f"{info['country']} · AQI {aqi_disp} · {aqi_lbl}</span>"
            f"</div>"
        )

        # FIX 1: bright white outline ring on every marker
        folium.CircleMarker(
            location=[info["lat"], info["lon"]],
            radius=12 if is_sel else 8,
            color="#ffffff",           # white outline on ALL markers
            weight=2.5 if is_sel else 2.0,
            fill=True,
            fill_color=col,
            fill_opacity=1.0 if is_sel else 0.90,
            tooltip=folium.Tooltip(tooltip_html, sticky=False),
            popup=folium.Popup(city, parse_html=False),
        ).add_to(m)

        # FIX 1: selected-city floating label — large, bold, readable
        if is_sel:
            folium.Marker(
                location=[info["lat"] + 0.20, info["lon"]],
                icon=folium.DivIcon(
                    html=(
                        f"<div style='"
                        f"font-family:\"Exo 2\",sans-serif;"
                        ...
                        f"background:rgba(26,26,46,0.85);"
                        f"border:1px solid rgba(80,160,255,0.45);"
                        f"border-radius:4px;"
                        f"padding:2px 7px;"
                        f"text-shadow:0 0 10px {col};"
                        f"box-shadow:0 0 10px rgba(0,0,0,0.60);"
                        f"'>{city}</div>"
                    ),
                    icon_size=(200, 26),
                    icon_anchor=(100, 0),
...
# ─────────────────────────────────────────────────────────────────────────────
#  PLOTLY CHART  —  lines only, NO fill, NO fillcolor
# ─────────────────────────────────────────────────────────────────────────────
def build_chart(df):
...
def render_who_panel(pm25_str, pm10_str, no2_str):
    def badge(val_str, limit):
        ...
        try:
            ...
            return '<span class="who-na">NO DATA</span>'
        return '<span class="who-good">✓ WITHIN</span>' if v <= limit else '<span class="who-bad">✗ ABOVE</span>'
...
# ─────────────────────────────────────────────────────────────────────────────
#  LAYOUT  —  FIX 2: map LEFT (col1), detail panel RIGHT (col2)
# ─────────────────────────────────────────────────────────────────────────────
col_map, col_data = st.columns([1, 1.8], gap="medium")
...
with col_map:
    ...
    st.markdown(
        '<div class="map-note">'
        '[ CLICK A MARKER TO LOAD FORECAST ] &nbsp;·&nbsp; '
    ...
    if not active_city:
        ...
        if no2_str != "—":
            no2_card = (
                ...
                f'<div class="mc-unit">µg/m³</div></div>'
            )
        ...
        <div class="aqi-block">
          <div class="aqi-row">
            <div class="aqi-orb"
                 style="background:{col_aqi};
                        box-shadow:0 0 18px {glow},0 0 36px {glow},
        ...
        st.markdown(
            ...
            f'<span class="scale-label-head">EU AQI SCALE</span>{chips}</div>',
            unsafe_allow_html=True,
        )

    # ── FIX 3: Pavan Clean Air website preview (always visible in right panel) ──
    st.markdown("""
    <div class="preview-panel">
      <div class="preview-titlebar">
        <span class="preview-dot" style="background:#ff5f57;"></span>
        <span class="preview-dot" style="background:#ffbd2e;"></span>
        <span class="preview-dot" style="background:#28ca41;"></span>
        <span class="preview-title">🌐 &nbsp; pavancleanair.com</span>
      </div>
      <div class="preview-body">
        <iframe
          src="https://www.pavancleanair.com"
          width="100%"
          height="350"
          frameborder="0"
          scrolling="yes"
          style="display:block;"
          title="Pavan Clean Air Website">
        </iframe>
      </div>
